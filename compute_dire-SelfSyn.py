...
def main():
    ...
    print('\r', 'total:' + str(args.num_samples), end='')
    ...
    while have_finished_images < args.num_samples:
        ...
        print(f"paths shape: {len(paths)}")
        print(args.real_step)
        ...
        print(f"\nsample_fn elapsed time: {elapsed_time:.2f}s")
        ...
        for i in range(len(recons)):
            ...
            # if fn_save.endswith('JPEG'):
                
            #     fn_save = fn_save.replace('JPEG', 'png')
            
            # elif fn_save.endswith('jpeg'):
            #     fn_save = fn_save.replace('jpeg', 'png')
            # elif fn_save.endswith('jpg'):
            #     fn_save = fn_save.replace('jpg', 'png')
            # elif fn_save.endswith('JPG'):
            #     fn_save = fn_save.replace('JPG', 'png')
            # elif fn_save.endswith('png'):
            #     continue
            if fn_save.endswith('png'):
                fn_save = fn_save.replace('png', 'jpg')
            # else:
            #     continue
            # print(recons_save_dir, fn_save, paths[i])
            # sys.exit()
        ...
        print('\r', 'count:' + str(have_finished_images), end='')        
...
def create_argparser():
    defaults = dict(
        images_dir="/data2/zhangjiehua/Dataset/AIGC/test/DALLE2",
        recons_dir="/data3/zhangjiehua/Dataset/Rec/test/DALLE2",
        dire_dir="/data3/zhangjiehua/Dataset/DIRE/test/DALLE2",
        clip_denoised=True,
        num_samples=12000,
        batch_size=16,
        use_ddim=True,
